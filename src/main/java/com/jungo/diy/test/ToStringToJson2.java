...
public class ToStringToJson2 {
    ...
    private static Map<String, Object> parseObject(String input) {
        ...
        String content = (input.endsWith(")")) ? input.substring(start + 1, input.length() - 1).trim() : input;

        List<String> tokens = splitKeyValuePairs(content);

        for (String token : tokens) {
            if (token.isEmpty()) continue;
            int eqIndex = findUnescapedEqualSign(token);
            ...
            if (value.matches("^[A-Za-z_][A-Za-z0-9_]*\\([^\\)]*\\)$")) {
                map.put(key, parseObject(value));  // 直接返回解析后的 Map
            } else if (value.startsWith("[") && value.endsWith("]")) {
                map.put(key, parseList(value));     // 返回解析后的 List
            } else {
                map.put(key, tryConvertValue(stripQuotes(value)));
            }
        }

        return map;  // 不再使用 singletonMap，直接返回解析后的 Map
    }


    private static int findUnescapedEqualSign(String token) {
        int depth = 0;
    ...
    private static List<Object> parseList(String input) {
        ...
        for (String el : elements) {
            if (el.matches("^[A-Za-z_][A-Za-z0-9_]*\\(.*\\)$")) {
                result.add(parseObject(el));  // 子对象解析为 Map
            } else {
                result.add(tryConvertValue(stripQuotes(el)));
            }
        }
